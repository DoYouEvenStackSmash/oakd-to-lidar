...
def find_obs(frame, lower_bound=50, upper_bound=350):
    ...
    conv = lambda x, h: np.apply_along_axis(
        lambda x: np.convolve(x, h.flatten(), mode="full"), axis=1, arr=x
    )
    ...
    for i in range(first_derivative.shape[0]):
        ...
        for j in range(filter_padding, first_derivative.shape[1] - filter_padding):
            if (
                np.sign(first_derivative[i, j]) != np.sign(first_derivative[i, j - 1])
                or first_derivative[i, j] < mean_arr[i] / 4
            ):
                obs_matrix[i, j:-1] = 0
                break
...
def get_markers(rect_arr):
    ...
    """
    markers = np.argmin(rect_arr[:, :] == 1, axis=1)
    return markers
